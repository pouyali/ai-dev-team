 ...
 * Note: DataProvider is expected to be wrapped at a higher level (e.g., root layout)
 */
export default function VolunteerLayout({ children }: VolunteerLayoutProps): JSX.Element {
  ...
  // For now, count pending requests as a proxy for notifications
  const unreadNotificationCount = requests.filter(req => req.status === 'pending').length;

  const tabs = [
    { id: 'requests', label: 'Requests', href: '/volunteer', icon: Home },
    { id: 'schedule', label: 'Schedule', href: '/volunteer/schedule', icon: Calendar },
    { id: 'notifications', label: 'Notifications', href: '/volunteer/notifications', icon: Bell, badge: unreadNotificationCount > 0 ? unreadNotificationCount : undefined },
    { id: 'reviews', label: 'Reviews', href: '/volunteer/reviews', icon: Star },
  ];

  // Determine active tab from pathname using startsWith for accurate matching
  const getActiveTab = (): string => {
    if (pathname === '/volunteer') return 'requests';
    if (pathname?.startsWith('/volunteer/schedule')) return 'schedule';
    if (pathname?.startsWith('/volunteer/notifications')) return 'notifications';
    if (pathname?.startsWith('/volunteer/reviews')) return 'reviews';
    return 'requests';
  };
